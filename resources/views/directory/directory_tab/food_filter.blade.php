<div class="filter">
    <h3>by Cuisine</h3>
    <form>
        <input type="checkbox" name="chinese" id="cbox1"><label for="cbox1"></label> Chinese <br>
        <input type="checkbox" name="malay" id="cbox2"><label for="cbox2"></label> Malay <br>
        <input type="checkbox" name="japanese" id="cbox3"><label for="cbox3"></label> Japanese <br>
        <input type="checkbox" name="koeran" id="cbox4"><label for="cbox4"></label> Koeran <br>
        <input type="checkbox" name="indian" id="cbox5"><label for="cbox5"></label> Indian <br>
        <input type="checkbox" name="Western" id="cbox6"><label for="cbox6"></label> Western <br>
    </form>

    <h3>by Meal Type</h3>
    <form>
        <input type="checkbox" name="breakfast" id="cbox7"><label for="cbox7"></label> Breakfast <br>
        <input type="checkbox" name="lunch" id="cbox8"><label for="cbox8"></label> Lunch <br>
        <input type="checkbox" name="tea-break" id="cbox9"><label for="cbox9"></label> Tea Break <br>
        <input type="checkbox" name="dinner" id="cbox10"><label for="cbox10"></label> Dinner <br>
        <input type="checkbox" name="supper" id="cbox11"><label for="cbox11"></label> Supper <br>
    </form>

    <h3>by Food Serve</h3>
    <form>
        <input type="checkbox" name="halal" id="cbox12"><label for="cbox12"></label> Halal <br>
        <input type="checkbox" name="non-halal" id="cbox13"><label for="cbox13"></label> Non-Halal <br>
        <input type="checkbox" name="vegetarian" id="cbox14"><label for="cbox14"></label> Vegetarian <br>
        <input type="checkbox" name="no-pork" id="cbox15"><label for="cbox15"></label> No Pork <br>
        <input type="checkbox" name="pork" id="cbox16"><label for="cbox16"></label> Pork <br>
    </form>
</div>